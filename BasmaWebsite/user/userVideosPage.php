...
if (!$result) {
    die("Video query error: " . mysqli_error($conn));
}
$videos = [];
while ($row = mysqli_fetch_assoc($result)) {
    $videoFile = basename(trim($row['video'] ?? ''));
    $posterFile = basename(trim($row['poster'] ?? ''));
    if ($videoFile === '') {
        continue;
    }
    $videoServerPath =
        __DIR__ . '/../uploads/uploadsVideos/' . $videoFile;
    if (!is_file($videoServerPath)) {
        continue;
    }
    $row['videoPath'] =
        '../uploads/uploadsVideos/' . rawurlencode($videoFile);
    $posterServerPath =
        __DIR__ . '/../uploads/uploadsVideoPosters/' . $posterFile;
    if ($posterFile !== '' && is_file($posterServerPath)) {
        $row['posterPath'] =
            '../uploads/uploadsVideoPosters/' . rawurlencode($posterFile);
    } else {
        $row['posterPath'] = '';
    }
    $videos[] = $row;
...
while ($row = mysqli_fetch_assoc($countResult)) {
    $categoryCounts[$row['categoryName']] = (int) $row['total'];
}
?>
<!DOCTYPE html>
